<template>
  <div class="bs-page-body">
    ...
    >
      <el-form-item>
        <span class="bs-form-label" style="width: 120px">
          <i class="icon">角色名称:</i>
        </span>
        <el-input
    ...
    <div class="bs-page-table">
      <div class="bs-table-btns">
        ...
        <el-button
          v-permission="{ action: 'role-delete' }"
          size="small"
          type="danger"
          icon="el-icon-delete"
        ...
        <el-button
          v-permission="{ action: 'role-setting' }"
          size="small"
          type="warning"
          icon="el-icon-setting"
          :disabled="selectedRows.length !== 1"
          @click="onClickAuthRole"
        >
          权限配置
        </el-button>
      </div>
      ...
      >
        ...
        <el-table-column label="备注" min-width="100">
          <template slot-scope="scope">
            <el-tooltip
              :visible-arrow="false"
              effect="light"
              :content="scope.row.overview"
              placement="bottom"
            >
              <span class="bs-tooltip-text"> {{ scope.row.overview }} </span>
            </el-tooltip>
          </template>
        </el-table-column>
        <el-table-column fixed="right" label="操作" width="220">
          <template slot-scope="scope">
            ...
            <el-dropdown>
              <el-button size="mini" :disabled="!isEditable(scope.row)">
              ...
              <el-dropdown-menu
                ...
                :disabled="!isEditable(scope.row)"
              >
                <el-dropdown-item @click.native="onClickAuthRole(scope.row)"
                  ><i class="el-icon-setting" />权限配置</el-dropdown-item
                >
              </el-dropdown-menu>
            </el-dropdown>
          </template>
    ...
    />
    <AuthDialog
      :is-show.sync="isShowDialogAuthRole"
      :dialog-data="authRoleData"
      @success="requestList"
      @closed="onClosedDialog"
    />
  </div>
...
export default {
  ...
  methods: {
    ...
    isEditable: function (row) {
      return !row.name.toLowerCase().includes("admin");
    },
    ...
    onClickDelete(row) {
      ...
      this.$confirm(
        ...
        if (row && row.id) {
          ...
          params.ids = this.selectedRows.map((item) => item.id); // 直接使用数组
        }
