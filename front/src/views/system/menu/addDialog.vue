      ...
      <div class="bs-dialog">
        ...
        >
          <el-form-item label="菜单类型:" prop="menuType">
            <el-select
              v-model="dialogFormData.menuType"
              placeholder="请选择菜单类型"
          ...
          <el-form-item
            v-if="dialogFormData.menuType !== 'catalog'"
            label="上级菜单:"
            prop="parentId"
          >
            <SingleSelectTree
              v-model.number="dialogFormData.parentId"
              placeholder="请选择上级菜单"
              :props="treeProps"
              :data="menuTreeData"
              @selected="getTreeValue"
            />
          </el-form-item>
          ...
          <el-form-item label="菜单编码:" prop="code">
            <template slot="label">
              ...
              <el-tooltip
                class="item"
                :content="
                  dialogFormData.menuType !== 'button'
                    ? '对应路由path和name'
                    : '定义的按钮权限标识'
                "
                effect="dark"
          ...
          <el-form-item
            v-if="dialogFormData.menuType === 'menu'"
            label="组件路径:"
            prop="component"
            class="input-width"
          >
          ...
          <el-form-item
            v-if="dialogFormData.menuType !== 'button'"
            label="状态:"
            prop="hidden"
          >
...
export default {
  ...
  methods: {
    ...
    getTreeValue(key, data) {
      ...
      this.dialogFormData.parentTitle = parsedData.title; // 设置 parentTitle 为选中节点的 parentTitle
      console.log(
    ...
    onChange() {
      ...
      this.dialogFormData.hidden = false;
      this.$refs["dialogFormRef"].clearValidate(); // 清空校验
    },
    submitRequest2() {
      ...
      const p = this.dialogType === "add" ? addMenu(params) : editMenu(params);
      p.then((res) => {
