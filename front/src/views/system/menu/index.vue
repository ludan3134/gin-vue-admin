  ...
  <div class="bs-page-body">
    ...
    >
      <el-form-item>
        ...
        <el-input
          ...
          maxlength="20"
          placeholder="根据菜单名称进行高亮显示"
          clearable
          @keyup.enter.native="requestList"
        />
      </el-form-item>
    </el-form>
    <!-- Search end -->
    ...
    <div class="bs-page-table">
      <div class="bs-table-btns">
        ...
        <el-button
          v-permission="{ action: 'menu-delete' }"
          size="small"
          type="danger"
          icon="el-icon-delete"
          :disabled="selectedRows.length !== 1"
      ...
      >
        ...
        <el-table-column label="菜单名称" min-width="180px">
          <template slot-scope="scope">
            <span
              :class="{ 'highlight-row': shouldHighlight(scope.row) }"
            ...
            >
              <a style="color: #00a0e9; text-decoration: underline">
                ...
                />
                <svg-icon
                  v-else-if="scope.row.icon.length"
        ...
        <el-table-column label="菜单类型" min-width="130">
          <template slot-scope="scope">
            <div :class="customStyle(scope.row)">
              {{ getDictLabel(scope.row.menuType, menuTypeOptions) }}
            </div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    shouldHighlight(row) {
      // 检查当前行是否包含查询参数
      // 例如，如果queryParams.title不为空且row.title包含queryParams.title
      return (
        this.queryParams.title && row.title.includes(this.queryParams.title)
      );
    },
    ...
    onClickDelete(row) {
      ...
      this.$confirm(
        `确定要删除菜单 ${title} （如果包含子菜单，将一并删除），此操作将永久删除, 是否继续?`,
        "提示",
        {
...
.highlight-row {
  background-color: #ffff2c; /* 这里你可以根据需要调整颜色 */
}
</style>
