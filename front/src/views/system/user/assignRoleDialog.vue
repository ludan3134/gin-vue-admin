              ...
              >
                ...
                <el-table-column prop="name" label="角色名称">
                  <template slot-scope="scope">
                    <span :class="{ highlight: scope.row.name === nameK }">{{
                      scope.row.name
                    }}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="overview" label="角色编号" />
              </el-table>
            </el-form>
          </el-col>
          <el-col :span="2">
          ...
          <el-col :span="11">
            ...
            <el-form :inline="true" size="small" class="role-right">
              ...
              >
                ...
                <el-table-column prop="name" label="角色名称">
                  <template slot-scope="scope">
                    <span :class="{ highlight: scope.row.name === nameY }">{{
                      scope.row.name
                    }}</span>
                  </template>
                </el-table-column>
...
<script>
import { getRoleListByUserId } from "@/api/system/user";
...
export default {
  ...
  methods: {
    ...
    highlightRows(type) {
      if (type === "K") {
        this.nameK = this.nameK; // 更新nameK的值
      } else if (type === "Y") {
        this.nameY = this.nameY; // 更新nameY的值
      }
    },
    requestList(type) {
    ...
    onClickArrowLeft() {
      ...
      this.tableDataY = this.tableDataY.filter(
        (item1) => !this.selectedRowsY.some((item2) => item1.id === item2.id)
      );
    },
    onClickArrowRight() {
      ...
      this.tableDataK = this.tableDataK.filter(
        (item1) => !this.selectedRowsK.some((item2) => item1.id === item2.id)
      );
    },
    onDialogSubmit() {
      this.submitRequest();
...
.highlight {
  background-color: #ffff99; /* 修改为更适合的背景颜色 */
}
</style>
